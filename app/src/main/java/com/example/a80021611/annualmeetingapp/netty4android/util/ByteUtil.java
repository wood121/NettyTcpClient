...
public class ByteUtil {
     ...
     */
    public static Request getResponseRequest(byte[] bytes) {
        if (!notNull(bytes)) {
            return null;
        }
        ...
        request.setLength(byteArrayToInt(b_length));

        byte[] b_serialNumber = Arrays.copyOfRange(bytes, 5, 9);
        request.setSerialNumber(byteArrayToInt(b_serialNumber));

        byte[] b_isNeedResponse = Arrays.copyOfRange(bytes, 9, 10);
        request.setIsNeedResponse(byteArrayToInt(b_isNeedResponse));

        byte[] b_sendId = Arrays.copyOfRange(bytes, 12, 14);
        request.setSendId(byteArrayToInt(b_sendId));

        byte[] b_receiveId = Arrays.copyOfRange(bytes, 14, 16);
        request.setReceiveId(byteArrayToInt(b_receiveId));

        byte[] b_commandPriority = Arrays.copyOfRange(bytes, 16, 17);
        request.setCommandPriority(byteArrayToInt(b_commandPriority));

        byte[] b_command_type = Arrays.copyOfRange(bytes, 17, 19);
        request.setCommandType(byteArrayToInt(b_command_type));

        byte[] b_commandContent = Arrays.copyOfRange(bytes, 19, length - 2);
        request.setCommandContent(b_commandContent);

        byte[] b_crc = Arrays.copyOfRange(bytes, length - 2, length);
        request.setVerifyCode(byteArrayToInt(b_crc));

        byte[] toCrc = Arrays.copyOfRange(bytes, 1, length - 2);
        request.setReponseVerifyCode(computeChecksum(toCrc, toCrc.length));

        return request;
    }

    public static int getNewVerifyCode(Request request) throws IOException {
        byte[] sendMsgByte = request.getSendMsgByte();
        if (notNull(sendMsgByte)) {
            if (request.getVerifyCode() != -1) {
